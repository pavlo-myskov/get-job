...
class TestSearchForm(TestCase):
    ...
    def test_form_field_max_lenght_validation(self):
        ...
        """
        search_query = "a" * 256
        form = JobSearchForm(data={"title": search_query})
    ...
    def test_form_fields_area_validation(self):
        ...
        invalid_form = JobSearchForm(data={"area": invalid_search_query})
        self.assertFalse(invalid_form.is_valid())

        valid_search_query = Areas.GALWAY_CITY
        valid_form = JobSearchForm(data={"area": valid_search_query})
    ...
    def test_form_fields_job_location_validation(self):
        ...
        invalid_form = JobSearchForm(
            data={"job_location": invalid_search_query}
        )
        self.assertFalse(invalid_form.is_valid())

        valid_search_query = Vacancy.JobLocations.REMOTE
        valid_form = JobSearchForm(data={"job_location": valid_search_query})
        self.assertTrue(valid_form.is_valid())
    ...
    def test_form_fields_job_type_validation(self):
        ...
        invalid_form = JobSearchForm(data={"job_type": invalid_search_query})
        self.assertFalse(invalid_form.is_valid())

        valid_search_query = Vacancy.JobTypes.FULL_TIME
        valid_form = JobSearchForm(data={"job_type": valid_search_query})
        self.assertTrue(valid_form.is_valid())
